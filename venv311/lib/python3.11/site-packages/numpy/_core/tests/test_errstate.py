...
class TestErrstate:
    ...
    def test_errstate_decorator(self):
        ...
        def foo():
            a = -np.arange(3)
            a // 0
<<<<<<< HEAD:venv311/lib/python3.11/site-packages/numpy/_core/tests/test_errstate.py

=======
            
>>>>>>> dd7ecbd (added imagen images):backend/venv/lib/python3.9/site-packages/numpy/_core/tests/test_errstate.py
        foo()
