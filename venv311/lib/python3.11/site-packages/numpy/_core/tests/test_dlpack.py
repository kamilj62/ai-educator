...
<<<<<<< HEAD:venv311/lib/python3.11/site-packages/numpy/_core/tests/test_dlpack.py
...
class TestDLPack:
    ...
    def test_dunder_dlpack_refcount(self):
        x = np.arange(5)
        y = x.__dlpack__()
>>>>>>> dd7ecbd (added imagen images):backend/venv/lib/python3.9/site-packages/numpy/_core/tests/test_dlpack.py
